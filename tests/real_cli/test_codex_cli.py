...
def gated_codex() -> tuple[str, str]:
    ...
    if not credential_id:
        pytest.fail(
            "AGENT_LOOP_CODEX_CREDENTIAL_ID is required when real-CLI probes are enabled"
        )
...
def test_066_pinned_prompt_input_probe_ignores_additional_root_instructions(
    ...
    (codex_home / "config.toml").write_bytes(SanitizedCodexConfig().render())
    ...
    before = {
        path: hashlib.sha256(path.read_bytes()).hexdigest() for path in markers
    }
